...
function UploadImages({ triggerUploadImages, setTriggerUploadImages, carInfo, mode }) {
    ...
    useEffect(()=>{
        console.log("Upload trigger changed:", triggerUploadImages);
    ...
    console.log('UploadImageToServer CALLED');  // Start log
    ...
    for (const file of selectedFileList) {
        console.log(`Uploading ${file.name} to: https://api.cloudinary.com/v1_1/${cloudName}/image/upload`);

        // Get signature and timestamp from your backend
        const signRes = await fetch('http://localhost:3001/api/cloudinary-sign', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
        // You can add body if you want to specify any options to server
        });
    ...
    if (setTriggerUploadImages) {
        setTriggerUploadImages(undefined);  // Reset parent's trigger so it can reactivate later
    }
    setLoader(false);
    };
