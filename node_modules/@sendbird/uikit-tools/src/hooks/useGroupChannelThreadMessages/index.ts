...
export const useGroupChannelThreadMessages = (
  ...
  useAsyncEffect(async () => {
    ...
    }) as FileMessage | UserMessage | undefined;
    if (parentMessage) {
      parentMessageRef.current = parentMessage;
      internalOptions.current.onParentMessageUpdated?.(parentMessage);
    }
  }, [channelRef.current?.url, internalOptions.current.isReactionEnabled]);
    ...
    return new Promise<void>((resolve) => {
      ...
      setTimeout(async () => {
        try {
          if (parentMessageRef.current) {
            ...
            const { threadedMessages } = await parentMessageRef.current.getThreadedMessagesByTimestamp(
              startingPoint,
              params,
            );
      ...
      collectionInstance.setMessageCollectionHandler({
        ...
        onMessagesUpdated: (ctx, __, messages) => {
          ...
          const parentMessage = messages.find((message) => message.messageId === parentMessageRef.current.messageId) as
            | FileMessage
            | UserMessage;
          if (parentMessage) {
            parentMessageRef.current = parentMessage;
  ...
  const updateUserMessage = usePreservedCallback(
    async (messageId: number, params: UserMessageUpdateParams): Promise<UserMessage> => {
      ...
      if (
        updatedMessage.channelUrl === channelRef.current.url &&
