  ...
  switch (action.type) {
    ...
    case 'update_messages': {
      if (action.value.clearBeforeAction) {
        ...
        const draftMessageMap = { ...draft['messageMap'] };
        const messagesToDelete: BaseMessage[] = [];
        ...
        messagesToDelete.forEach((it) => {
          if (isSendableMessage(it)) delete draftMessageMap[it.reqId];
          delete draftMessageMap[it.messageId];
        });

        messagesToAdd.forEach((it) => {
          if (isSendableMessage(it)) draftMessageMap[it.reqId] = it;
...
const shouldUpdateMessage = (prev: BaseMessage, next: BaseMessage, strictStreamingOrder = false) => {
  if (!isSendableMessage(prev) || !isSendableMessage(next)) return true;

  // When strictStreamingOrder is enabled (for AI streaming), check message length
  if (strictStreamingOrder && prev) {
    const prevLength = prev.message?.length || 0;
    const nextLength = next.message?.length || 0;
