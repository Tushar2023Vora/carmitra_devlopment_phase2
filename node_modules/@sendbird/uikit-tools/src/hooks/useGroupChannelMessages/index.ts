...
type UseGroupChannelMessagesOptions = {
  ...
  onChannelUpdated?: (channel: GroupChannel, ctx?: GroupChannelEventContext) => void;
  onCurrentUserBanned?: () => void;

  logger?: { info?: Log; warn?: Log; error?: Log; log?: Log; debug?: Log };

  /**
   * When true, prevents message updates with shorter content to handle out-of-order streaming updates.
   * Used for AI agent message streaming to avoid message flicker or truncation when updates arrive out of sequence.
   * Example: "Hello world!" → "Hello wor" → "Hello world!" → "Hel" would display "Hello world!" instead of "Hel"
   * Only applies to sendable messages with SUCCEEDED status.
   * @default false
   */
  strictStreamingOrder?: boolean;
};
...
export const useGroupChannelMessages = (
  ...
  const strictStreamingOrder = internalOptions.current.strictStreamingOrder ?? false;
  ...
  const init = usePreservedCallback(
    async (channel: GroupChannel, startingPoint: number, limit: { next: number; prev: number }) => {
      return new Promise<void>((resolve, reject) => {
        ...
        const collectionInstance = createMessageCollection(channel, limit, {
        ...
        collectionInstance.setMessageCollectionHandler({
          onMessagesAdded: (ctx, __, messages) => {
            ...
            updateMessages(messages, false, sdk.currentUser?.userId, strictStreamingOrder);
          },
          ...
          onChannelUpdated: (ctx, channel) => {
            forceUpdate();
            internalOptions.current.onChannelUpdated?.(channel, ctx);
          },
          onHugeGapDetected: () => {
        ...
        collectionInstance
          ...
          .onCacheResult((err, messages) => {
            if (err) {
              sdk.isCacheEnabled && logger?.error?.('[useGroupChannelMessages/onCacheResult]', err);
          ...
          .onApiResult((err, messages) => {
            ...
            if (err) {
              logger?.warn?.('[useGroupChannelMessages/onApiResult]', err);
  ...
  useLayoutEffect(() => {
    const timeout = setTimeout(async () => {
      if (sdk.currentUser && channelRef.current) {
        ...
        updateLoading(true);
        await init(channelRef.current, initialStartingPoint, initialLimit);
        updateLoading(false);
  ...
  const refresh = usePreservedCallback(async () => {
    if (sdk.currentUser && channelRef.current) {
      ...
      await init(channelRef.current, Number.MAX_SAFE_INTEGER, sbuConstants.collection.message.defaultLimit);
  ...
  const resetWithStartingPoint = usePreservedCallback(async (startingPoint: number) => {
    if (sdk.currentUser && channelRef.current) {
      ...
      await init(channelRef.current, startingPoint, limit);
      updateLoading(false);
    }
  });
